...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private IEnumerator DoAction(ConvaiAction action)
        {
            ...
            switch (action.Verb)
            {
                ...
                case ActionChoice.Hit:
                    // Call the Crouch function and yield until it's completed
                    yield return Hit(action.Target);
                    break;

                case ActionChoice.Link:
                    // Call the Crouch function and yield until it's completed
                    yield return Link(action.Target);
                    break;
        ...
        {
            // 开始播放行走动画
            _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Walking"), 0.05f);

            // 计算方向并开始朝目标移动
            Vector3 targetPosition = target.transform.position;
            while (Vector3.Distance(_currentNPC.transform.position, targetPosition) > 0.1f)
            {
                // 朝目标位置移动
                Vector3 direction = (targetPosition - _currentNPC.transform.position).normalized;
                _currentNPC.transform.position += direction * Time.deltaTime * 2.0f; // 根据需要调整速度

                // 旋转以面对移动方向
                Quaternion targetRotation = Quaternion.LookRotation(direction);
                _currentNPC.transform.rotation = Quaternion.Slerp(_currentNPC.transform.rotation, targetRotation, Time.deltaTime * 5.0f);

                yield return null;
            }

            // 确保 NPC 精确到达目标位置
            _currentNPC.transform.position = targetPosition;

            // 旋转以面向目标的 Z 轴方向
            Vector3 lookDirection = target.transform.forward; // 使用目标物体的正前方向来设定最终朝向
            lookDirection.y = 0; // 保持旋转只在水平面上
            if (lookDirection != Vector3.zero)
            {
                _currentNPC.transform.rotation = Quaternion.LookRotation(lookDirection);
            }

            // 播放跳跃动画
            _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Hit"), 0.05f);
            yield return new WaitForSeconds(1.0f); // 根据跳跃动画的持续时间调整等待时间

            // 添加功能：跳跃后面向tag为"player"的GameObject
            GameObject player = GameObject.FindWithTag("Player");
            if (player != null)
            {
                Vector3 playerDirection = player.transform.position - _currentNPC.transform.position;
                playerDirection.y = 0; // 确保只在水平面上旋转
                if (playerDirection != Vector3.zero)
                {
                    _currentNPC.transform.rotation = Quaternion.LookRotation(playerDirection);
                }
            }

            // 跳跃后恢复到空闲动画
            _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Talking"), 0.05f);
            yield return null;
        }

        private IEnumerator Link(GameObject target)
        {
            // 开始播放行走动画
            _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Walking"), 0.05f);
            ...
            // 播放跳跃动画
            _currentNPC.GetComponent<Animator>().CrossFade(Animator.StringToHash("Sit"), 0.05f);
            yield return null;
        }
